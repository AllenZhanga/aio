...
### 4.0 架构细化补充：变量、属性配置与节点新增规则

本节记录 2026-04-30 对 Workflow 架构的进一步细化，作为后续实现前的约束依据。

#### 4.0.1 每个节点都必须具备变量读取能力

每个节点的所有可配置输入字段，都应能够读取统一变量上下文。变量来源分为五类：

| 变量来源 | 命名空间 | 示例 | 可见范围 |
| --- | --- | --- | --- |
| 系统变量 | `sys` | `{{sys.runId}}`、`{{sys.workspaceId}}` | 所有节点可读，部分字段只读。 |
| 流程输入 | `inputs` | `{{inputs.question}}` | 所有非 Start 节点可读；Start 节点负责定义和初始化。 |
| 流程变量 | `vars` | `{{vars.customerId}}`、`{{vars.priority}}` | 所有节点可读；由变量节点或输出映射写入。 |
| 前置节点变量 | `nodes` | `{{nodes.answer.text}}`、`{{nodes.retrieve.chunks}}` | 只允许读取当前节点的上游节点输出。 |
| 调用元数据 | `metadata` | `{{metadata.requestId}}`、`{{metadata.userAgent}}` | 所有节点可读，来自调用端或运行环境。 |

设计约束：

1. 节点配置不直接读全局对象，统一通过变量引用组件插入引用表达式。
2. 前端变量树必须根据 DAG 关系过滤，只展示当前节点可合法引用的上游变量。
3. 发布检查必须阻断下游引用、循环引用、未知节点、未知字段和未知命名空间。
4. 变量引用的解析规则前后端必须一致，避免设计期可选但运行期失败。
5. 节点输出字段必须来自 NodeSpec 的 `outputSchema` 或该节点的显式输出映射。
6. 删除节点、删除连线、重命名节点 ID 或修改输出字段时，必须触发引用影响分析。

#### 4.0.2 标准变量引用输入组件

节点配置页面必须使用标准变量引用组件，不能让每个节点自行拼 textarea。建议组件命名为 `VariableReferenceInput` 或 `WorkflowVariableInput`，由工作流模块统一维护。

组件能力：

1. 支持纯文本、变量引用、表达式、Markdown、富文本、JSON 等模式。
2. 支持点击插入变量，变量以 `{{namespace.path}}` 形式写入光标位置。
3. 支持变量树弹层，按“系统变量 / 流程输入 / 流程变量 / 上游节点输出 / 元数据”分组。
4. 支持搜索变量名称、节点名称、字段说明。
5. 支持变量 token 高亮，不把变量引用仅当普通字符串展示。
6. 支持字段类型提示，例如 string、number、boolean、array、object。
7. 支持非法引用提示，例如“该变量来自下游节点，当前节点不可引用”。
8. 支持不同字段配置可用模式，例如 URL 字段只允许文本和变量，Prompt 字段允许 Markdown 和变量，JSON 字段允许 JSON path 插值。
9. 支持插入常用片段，例如 `{{inputs.question}}`、`{{nodes.answer.text}}`。
10. 支持后续扩展自动补全，但第一版可以先做变量选择弹层和插入。

组件输入建议：

```ts
type VariableReferenceInputProps = {
  value: string;
  mode: "text" | "expression" | "markdown" | "richText" | "json";
  currentNodeId?: string;
  availableVariables: WorkflowVariableOption[];
  expectedType?: WorkflowValueType;
  placeholder?: string;
...
`availableVariables` 不应该由输入组件自己遍历全图生成，而应由 `workflowGraph.ts` 或变量服务根据当前节点统一计算。这样可以保证所有配置字段、连线条件、输出映射使用同一套可见性规则。

#### 4.0.3 中间新增节点时禁止新增开始节点

开始节点是流程唯一入口，不应出现在节点出口 `+`、连线中点 `+`、拖线到空白处新增等中间新增场景。

规则：

1. `start` 只能在新建 Workflow 模板时自动创建。
2. 节点库可以展示 Start 的说明，但不允许用户手动新增第二个 Start。
3. `NodeAddMenu` 在以下场景必须过滤 `start`：
   - 节点出口 `+` 新增后继。
   - 连线中点 `+` 插入节点。
   - 从输出 Handle 拖到空白处新增节点。
   - 从节点库拖到已有流程中间位置。
4. 发布检查必须要求且只允许一个 Start 节点。
5. 所有非 Start 节点必须能从 Start 可达；所有有效路径最终应能到达 End 或明确的等待/终止策略。
6. End 节点可以新增多个还是唯一，需要产品确认。第一版建议允许多个 End，但每条执行路径必须能到达至少一个 End。

#### 4.0.4 节点参数配置需要按“任务模型”重新设计

当前按字段堆叠输入框的体验不足以支撑真实使用。每类节点都应从“用户要完成的任务”出发设计参数，而不是直接暴露后端字段。

通用原则：

1. 基础信息少放：节点名称、启用状态、说明即可。
2. 常用配置优先：默认展开最常用的 3 到 5 个参数，低频配置进入高级区。
3. 输入映射独立成区：用户先决定这个节点吃什么数据，再配置节点行为。
4. 输出设计独立成区：用户能看到本节点会产出什么字段，以及哪些字段会写入 `vars`。
5. 复杂字段要有结构化编辑器：JSON Schema、HTTP Headers、表单字段、动作按钮不能只给 raw textarea。
6. 所有变量输入都使用标准变量引用组件。
7. 每个字段要有即时校验、空值提示和示例值。
8. 节点配置区要展示“当前节点可引用变量”，降低用户记忆成本。
9. 高级区用于节点 ID、内部类型、超时、重试、缓存、原始 JSON。
10. 节点配置错误要能回显到画布节点卡片和发布检查面板。

第一版各节点配置方向：

| 节点 | 配置重点 | 需要避免 |
| --- | --- | --- |
| 开始 | 流程输入定义：字段名、类型、必填、默认值、说明、示例 | 不把 Start 当普通执行节点配置。 |
| 变量赋值 | 写入 `vars` 的目标变量、值来源、类型转换、覆盖策略 | 不只提供一个 `result` 字符串。 |
| 大模型 | 模型选择、Prompt、上下文变量、温度、最大输出、结构化输出 | 不让用户手写所有 provider/model/config JSON。 |
| 智能体 | 选择目标 Agent、输入映射、运行模式、输出映射 | 不要求用户记忆 appId。 |
| 知识检索 | 选择知识库、检索问题、TopK、阈值、过滤条件、返回字段 | 不要求用户手填 datasetId。 |
| 工具调用 | 选择工具、自动渲染工具入参表单、输出字段说明 | 不只暴露 toolId 和 input textarea。 |
| HTTP 请求 | Method、URL、Headers 表格、Query 表格、Body 编辑器、鉴权 | 不把 headers/body 都混进一个文本框。 |
| 条件分支 | 多条件规则、默认分支、表达式预览、可引用变量 | 不只给一个 expression 字段。 |
| 代码执行 | 语言、输入变量、代码编辑器、返回字段约定、沙箱限制 | 不把代码与普通 Markdown textarea 混用。 |
| 人工确认 | 标题、说明、动作按钮、超时、处理人/角色、恢复分支 | 不把 actions 隐藏在 JSON 里。 |
| 人工表单 | 表单字段设计器、字段校验、提交说明、超时、处理人/角色 | 不要求用户直接写 formSchema。 |
| 结束 | 输出映射、返回格式、失败时输出策略 | 不只给一个 output 字符串。 |

#### 4.0.5 还需要补充的能力

在上述五点之外，Workflow 设计器还需要补充以下能力，建议纳入后续规划：

1. 变量影响分析：当节点或变量被修改时，列出受影响的下游字段和连线条件。
2. 节点模板：常见配置可保存为模板，例如“知识检索 + 大模型回答”。
3. 配置预览：每个节点能预览“解析变量后的实际入参”，避免运行时才发现拼接错误。
4. 单节点试运行：在属性弹窗内对当前节点用样例上下文试跑。
5. 输出样例管理：节点保存最近一次成功输出，作为下游变量选择和类型推断依据。
6. 条件分支可视化：条件节点的 true/false/else 或多条件分支要在端口和边标签上可见。
7. 权限与密钥边界：`env` 和 secret 只能引用，不能在前端明文展示。
8. 错误恢复策略：节点级失败处理、重试、跳过、走错误分支需要统一配置入口。
9. 复制/粘贴节点时变量引用处理：复制节点应生成新 ID，并提示是否重定向引用。
10. 导入/导出 DSL：高级用户可导出 JSON，但导入时必须经过同一套校验。
11. 可访问性和键盘操作：删除、复制、撤销、重做、打开属性、关闭弹窗需要键盘支持。
12. 节点新增过滤：除 Start 外，还应根据上下文过滤非法节点，例如 End 后不能追加后继节点。
13. 表单状态持久化：属性弹窗关闭不应丢失未保存编辑；草稿自动保存失败要明确提示。
14. 版本兼容策略：DSL `version` 升级时需要 migration 或明确不兼容边界。
